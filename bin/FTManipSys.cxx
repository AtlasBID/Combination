...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    for (int i = 1; i < argc; i++) {
      ...
      else if (a == "dropSysError") {
        dropSysError = eatArg(argv, i, argc);
      }
      else {
    ...
    if (newsysval == "" && relDifAna1 == "" && dropSysError.size() == 0) {
    ...
    else if (dropSysError.size() > 0) {
      for (size_t i = 0; i < info.Analyses.size(); i++) {
        CalibrationAnalysis ana(info.Analyses[i]);
...
void Usage(void)
{
  ...
  cout << "  dropSysError <oldsys>               - Drop a sys error" << endl;
